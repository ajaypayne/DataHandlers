...
foreach ($xml as $envName => $env) {
    ...
    foreach ($env as $syndicateName => $details) {
        foreach ($details as $name => $dnis) {
            $newXmlString .= '<transferMap>';
            $newXmlString .= '<map id="' . $dnis[0] . '">';
            $newXmlString .= '<serviceName>LauraAshleyIVRCallGuard-' . $syndicateName . '</serviceName>';
            $newXmlString .= '<promptPath>english</promptPath> <!-- folder for syndication prompts remove for default-->';
            $newXmlString .= '<serviceId>25537</serviceId>';
            $newXmlString .= '<defaultPaymodServiceId>25537</defaultPaymodServiceId>';
//            if (preg_match('/INBOUND(_\d)?(\d)?/', $syndicateName)) {
                $newXmlString .= '<callGuard>';
                $newXmlString .= '<destination>changeMe' . $syndicateName . '</destination>';
                $newXmlString .= '<drDestination />';
                $newXmlString .= '</callGuard>';
                $newXmlString .= '<transfer>';
                $newXmlString .= '<numbers>';
                $newXmlString .= '<ClientCallCenterNumber>changeMe' . $syndicateName . '</ClientCallCenterNumber>';
                $newXmlString .= '</numbers>';
                $newXmlString .= '</transfer>';
                $newXmlString .= '<configurationFile>INBOUND</configurationFile>';
//            } else {
//                $newXmlString .= '<configurationFile>OUTBOUND</configurationFile>';
//            }
            $newXmlString .= '</map>';
            $newXmlString .= '</transferMap>';
        }
